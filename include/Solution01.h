...
class UnionFind_leetcode399 // 并查集、维护权值
{
   public:
    ...
    void Union(int a, int b,
               std::unordered_map<int, std::unordered_map<int, double>>& ans);
...
class Solution {
   public:
    ...
    std::string addStrings(std::string num1,
                           std::string num2); // leetcode 415 9/22/22

    std::string multiply(std::string num1,
                         std::string num2); // leetcode 43 9/22/22
    ...
    int maximumRemovals(std::string s, std::string p,
                        std::vector<int>& removable); // leetcode 1898 9/20/22
    bool sub_1898(int k, std::vector<int>& removable, std::string s,
                  std::string p);

    // std::vector<double> calcEquation(std::vector<std::vector<std::string>>&
    // equations, std::vector<double>& values,
    // std::vector<std::vector<std::string>>& queries);	// leetcode 399
    // 9/16-17/22
    ...
    // int numIslands(std::vector<std::vector<char>>& grid);	// leetcode 200
    // 9/14/22 三个思路 bfs dfs 并查集
    void bfs(std::vector<std::vector<char>>& grid,
             std::deque<std::pair<int, int>>& coordinates, int m, int n);
    // dfs 与 bfs 类似
    int numIslands(std::vector<std::vector<char>>& grid); // UnionFind *

    // vector<int> nextGreaterElement(vector<int>& nums1, vector<int>& nums2);
    // // leetcode 496 9/13/22
    std::vector<int> nextGreaterElement(std::vector<int>& nums1,
                                        std::vector<int>& nums2); // 单调栈解法

    int chalkReplacer(std::vector<int>& chalk, int k); // leetcode 1894 9/12/22
    // int chalkReplacer(std::vector<int>& chalk, int k);

    int minSpeedOnTime(std::vector<int>& dist,
                       double hour); // leetcode 1870 9/10/22
    bool judgeSpeed(std::vector<int>& dist, double hour, int speed);

    int networkDelayTime(std::vector<std::vector<int>>& times, int n,
                         int k); // leetcode 743 9/9/22 9/18/22

    bool containsCycle(
        std::vector<std::vector<char>>& grid); // leetcode 1559 9/8/22 ***

    // int cherryPickup(std::vector<std::vector<int>>& grid);	// leetcode 741
    // 没写出来

    std::vector<std::vector<int>> floodFill(
        std::vector<std::vector<int>>& image, int sr, int sc,
