...
string Solution01::addStrings(string num1, string num2) {	// leetcode 415 9/22/22
...
string Solution01::multiply(string num1, string num2) {		// leetcode 43 9/22/22
...
//string Solution01::multiply(string num1, string num2) {
...
//	return ans;
//}
//void Solution01::mul(string A, int b, int i, vector<int>& res) {
...
vector<int> Solution01::sortArray(vector<int>& nums) {	// leetcode 912 9/21/22
...
void Solution01::maxHeap(vector<int>& nums, int i, int len)
{
	while (true)
	{
...
int Solution01::maximumRemovals(string s, string p, vector<int>& removable) {	// leetcode 1898 9/20/22
...
bool Solution01::sub_1898(int k, vector<int>& removable, string s, string p)
{
...
//vector<double> Solution01::calcEquation(vector<vector<string>>& equations, vector<double>& values, vector<vector<string>>& queries)	// leetcode 399 9/16-17/22
//{
...
vector<double> Solution01::calcEquation(vector<vector<string>>& equations, vector<double>& values, vector<vector<string>>& queries) {
...
string Solution01::find(string x) {	// 并查集
	if (p[x] != x) {
		string origin = p[x];
		p[x] = find(p[x]);
...
//int Solution01::numIslands(vector<vector<char>>& grid)	// leetcode 200 9/14/22 三个思路 bfs dfs 并查集
//{
...
//	return count;
//}
void Solution01::bfs(vector<vector<char> >& grid, deque<pair<int, int> >& coordinates, int m, int n)
{
	int x, y;
...
// dfs 与 bfs 类似
int Solution01::numIslands(vector<vector<char>>& grid)	// UnionFind *
{
...
//vector<int> Solution01::nextGreaterElement(vector<int>& nums1, vector<int>& nums2) {	// leetcode 496 9/13/22
...
//	return result;
//}
vector<int> Solution01::nextGreaterElement(vector<int>& nums1, vector<int>& nums2) {	// 单调栈解法
...
int Solution01::chalkReplacer(vector<int>& chalk, int k) {	// leetcode 1894 9/12/22
...
//int Solution01::chalkReplacer(vector<int>& chalk, int k) {
//	int n = chalk.size();
...
int Solution01::minSpeedOnTime(vector<int>& dist, double hour) {	// leetcode 1870 9/10/22
	int n = dist.size();
	if (n - 1 >= hour)
		return -1;
...
bool Solution01::judgeSpeed(vector<int>& dist, double hour, int speed)
{
	int n = dist.size();
...
int Solution01::networkDelayTime(vector<vector<int>>& times, int n, int k) {	// leetcode 743 9/9/22 9/18/22
...
bool Solution01::containsCycle(vector<vector<char>>& grid) {	// leetcode 1559 9/8/22 ***
	int m = grid.size(), n = grid[0].size();
	if (m == 1 || n == 1)
		return false;
...
vector<vector<int>> Solution01::floodFill(vector<vector<int>>& image, int sr, int sc, int color) {	// leetcode 733 9/5/22
...
//vector<vector<int>> Solution01::floodFill(vector<vector<int>>& image, int sr, int sc, int color) {
...
int Solution01::minPathSum(vector<vector<int>>& grid) {	// leetcode 64	// 进阶版，允许各个方向的移动 *
	int m = grid.size(), n = grid[0].size();
...
vector<vector<int>> Solution01::generate(int numRows) {	// leetcode 118
...
//vector<vector<int>> Solution01::generate(int numRows) {	// 参考答案
...
vector<int> Solution01::searchRange(vector<int>& nums, int target) {	// leetcode 34 *
	int len = nums.size();
...
int Solution01::minCostClimbingStairs(vector<int>& cost) {	// leetcode 746
	int len = cost.size();
...
int Solution01::climbStairs(int n) {	// leetcode 70
	if (n <= 2)
		return n;
...
void Solution01::printVector(vector<vector<int> >& nums)
{
