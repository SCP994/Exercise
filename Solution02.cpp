...
vector<int> Solution02::getModifiedArray(int length, vector<vector<int> >& updates)
{
...
vector<int> Solution02::getModifiedArray_(int length, vector<vector<int> >& updates)
{
...
bool Solution02::possibleToStamp(vector<vector<int> >& grid, int stampHeight, int stampWidth)
{
...
int Solution02::lengthOfLongestSubstring(string s)
{
...
int Solution02::numSubarrayProductLessThanK(vector<int>& nums, int k)
{
...
int Solution02::numSubarrayProductLessThanK_(vector<int>& nums, int k)
{
...
int Solution02::hammingWeight(uint32_t n)
{
    int count = 0;
    while (n > 0)
    {
...
int Solution02::hammingWeight_(uint32_t n)
{
    int count = 0;
    while (n)
    {
...
int Solution02::hammingWeight__(uint32_t n)
{
    int count = 0;
    while (n)
    {
...
vector<vector<int> > Solution02::merge(vector<vector<int> >& intervals)
{
...
vector<vector<int> > Solution02::merge_(vector<vector<int> >& intervals)
{
    int len = intervals.size();
    vector<vector<int> > result;
...
vector<int> Solution02::dailyTemperatures(vector<int>& temperatures)
{
...
vector<int> Solution02::dailyTemperatures_(vector<int>& temperatures)
{
    int len = temperatures.size();
    vector<int> result(len);
    stack<int> temp;    // 只存索引
...
int Solution02::sumSubarrayMins(vector<int>& arr)
{
    int len = arr.size();
...
int Solution02::maxWidthRamp(vector<int>& nums)
{
...
int Solution02::maxWidthRamp_(vector<int>& nums)
{
    int len = nums.size(), width = 0;
    stack<int> temp;
    temp.push(0);
...
int Solution02::maxChunksToSorted(vector<int>& arr)
{
...
int Solution02::maxChunksToSorted_(vector<int>& arr)
{
    stack<int> temp;
    for (auto& i : arr)
...
long long Solution02::subArrayRanges(vector<int>& nums)
{
    int len = nums.size();
...
int Solution02::maxSumMinProduct(vector<int>& nums)
{
    int len = nums.size();
...
vector<int> Solution02::maxSlidingWindow(vector<int>& nums, int k)
{
    int len = nums.size();
...
vector<int> Solution02::maxSlidingWindow_(vector<int>& nums, int k)
{
    int len = nums.size();
    deque<int> q;   // 双端队列
    vector<int> result;
...
int Solution02::findMaxValueOfEquation(vector<vector<int> >& points, int k)
{
...
int Solution02::findMaxValueOfEquation_(vector<vector<int> >& points, int k)
{
...
int Solution02::shortestSubarray(vector<int>& nums, int k)
{
...
int Solution02::constrainedSubsetSum(vector<int>& nums, int k)
{
    int len = nums.size(), result = -inf;
    vector<int> dp(len);
    deque<int> q;
...
string Solution02::shortestPalindrome(string s)
{
    int len = s.length();
    string result;
    for (int i = len; i; --i)
...
string Solution02::shortestPalindrome_(string s)
{
...
vector<vector<int> > Solution02::palindromePairs(vector<string>& words)   // 字符串哈希，效率一般
{
...
bool Solution02::isPalindrome(string& str, int left, int right)
{
...
vector<vector<int> > Solution02::palindromePairs_(vector<string>& words)
{
    TrieTree trieTree;
    int len = words.size();
    for (int i = 0; i < len; ++i)
...
vector<pair<int, int> > Solution02::manacher(string& s)
{
    int len = s.size();
    string temp = "#";
    temp += s[0];
...
vector<vector<int> > Solution02::palindromePairs__(vector<string>& words)
{
...
string Solution02::longestDupSubstring(string s)
{
...
int Solution02::distinctEchoSubstrings(string text)
{
...
bool Solution02::dfs_291(int i, int j, string& p, string& s, unordered_set<string>& vis, unordered_map<char, string>& d)
{
    int m = p.size(), n = s.size();
    if (i == m && j == n) return true;
    if (i == m || j == n || m - i > n - j) return false;    // 不考虑一个字符对应空串的情况
    char c = p[i];
    for (int k = j + 1; k <= n; ++k)
    {
        string t = s.substr(j, k - j);
        if (d.count(c) && d[c] == t)
            if (dfs_291(i + 1, k, p, s, vis, d)) return true;
...
bool Solution02::wordPatternMatch(string pattern, string s)
{
