...
namespace s04 {
Solution::Solution() : m(0), n(0), inf(0x3f3f3f3f) {}
...
int Solution::cherryPickup(vector<vector<int>>& grid) {
    ...
    vector<vector<vector<int>>> dp(
        n * 2, vector<vector<int>>(n, vector<int>(n, -inf)));
    dp[0][0][0] = grid[0][0];
...
int Solution::cherryPickup_(vector<vector<int>>& grid) {
    ...
    vector<vector<vector<int>>> dp(
        m,
        vector<vector<int>>(n, vector<int>(n, -inf))); // 每个机器人最多走 m 步
            ...
            for (int j2 = n - 1 - steps; j2 < n; ++j2) {
                ...
                for (int j1_up = j1 - 1; j1_up <= j1 + 1; ++j1_up)
                    for (int j2_up = j2 - 1; j2_up <= j2 + 1; ++j2_up)
                        if (j1_up < 0 || j2_up < 0 || j1_up >= n || j2_up >= n)
                            continue;
                        else
                            dp[steps][j1][j2] =
                                max(dp[steps][j1][j2],
                                    ret + dp[steps - 1][j1_up][j2_up]);
...
MyBITree::MyBITree(int len) : BinaryIndexedTree(len) {}
...
int Solution::eraseOverlapIntervals_(
    std::vector<std::vector<int>>& intervals) // O(n^2) 超时
{
    int n = intervals.size();
    sort(intervals.begin(), intervals.end());
    vector<int> dp(n,
                   1); // 以区间 i 为最后的一个区间，可以选出的区间数量的最大值
    for (int i = 1; i < n; ++i)
        for (int j = 0; j < i; ++j)
