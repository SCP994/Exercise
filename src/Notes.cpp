...
void check() {
    int i =
        0b00000001000000100000010000001000; // 如果是大端存储，数据低位保存到地址高位；小端存储，数据低位保存到地址低位
...
float testTime() {
    ...
    float t_total =
        chrono::duration<float, std::milli>(t_end - t_start).count();
    std::cout << "1 detection and track total take: " << t_total << " ms."
              << std::endl;
    return t_total;
}
...
void post_order(TreeNode* node) {
    ...
    while (!s.empty()) {
        ...
        if (!p->right ||
            temp ==
                p->right) // 不存在右子树或者右子树被访问过，则打印当前结点并弹出
        {
            cout << p->value << endl;
            temp = p;
            s.pop();
