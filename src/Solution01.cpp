...
namespace s01 {
MyLinkedList::MyLinkedList() : m_head(new Node()), m_len(0) {}
...
void UnionFind_leetcode399::Union(
    int a, int b, unordered_map<int, unordered_map<int, double>>& ans) {
    if (parent[a] == a)
        parent[a] = b;
    else {
        int temp = Find(a, ans);
        parent[temp] = b;
        ans[temp][b] = ans[temp][a] *
                       ans[a][b]; // 确保 ans[a][b] 再 Union 过程之前已被赋值
        ans[b][temp] = 1.0 / ans[temp][b];
    }
}

int UnionFind_leetcode399::Find(
    int a, unordered_map<int, unordered_map<int, double>>& ans) { // 路径压缩
    if (parent[a] == a)
        return a;
    else {
    ...
    for (int i = 0; i < len1; ++i)
        for (int j = 0; j < len2; ++j)
            sums[i + 1][j + 1] = sums[i + 1][j] + sums[i][j + 1] - sums[i][j] +
                                 matrix[i][j]; // careful here
}

int NumMatrix::sumRegion(int rol1, int col1, int rol2, int col2) {
    return sums[rol2 + 1][col2 + 1] - sums[rol2 + 1][col1] -
           sums[rol1][col2 + 1] + sums[rol1][col1];
}

string Solution::addStrings(string num1, string num2) { // leetcode 415 9/22/22
    int len1 = num1.length(),
        len2 = num2.length(), // 算出来的结果可以先存到 vector<int> 里面
    ...
    for (int i = 0; i < len_min; ++i) {
        auto it =
            adder(num1[len1 - i - 1] - '0', num2[len2 - i - 1] - '0', c_in);
        c_in = it.second;
...
string Solution::multiply(string num1, string num2) { // leetcode 43 9/22/22
    int len1 = num1.length(), len2 = num2.length(), lenMin = min(len1, len2),
        lenMax = max(len1, len2);
    int c_in = 0;
    string result = "0", mid = "", strMax = len1 >= len2 ? num1 : num2,
           strMin = len1 >= len2 ? num2 : num1;
...
int Solution::maximumRemovals(string s, string p,
                              vector<int>& removable) { // leetcode 1898 9/20/22
    int min = 0, max = removable.size();
    while (
        min <
        max) // 注意要考虑等于的情况，比如 0 - 10 中找 7
             ...
             // 这题不同的是，min = 0 永远符合条件，min
             // 只会取符合条件的值，最后返回 min，有等于号会死循环
             // 这道题类似与二分查找第一个小于指定值的值的索引，假设不存在相等的值。但当数组中
             // 没有小于的值时会返回索引 0，答案错误，而这道题 0
             // 永远成立，所以不存在这个问题
    {
        int mid = (min + max + 1) >> 1;
        if (sub_1898(mid, removable, s, p))
            ...
            max = mid - 1; // 注意这里的逻辑，符合的 mid 赋值给 min
                           // ，不符合，max = mid - 1，
    }
    return min;
}
...
// vector<double>& values, vector<vector<string>>& queries)	// leetcode 399
// 9/16-17/22
//{
...
// int Solution::numIslands(vector<vector<char>>& grid)	// leetcode 200 9/14/22
// 三个思路 bfs dfs 并查集
//{
...
//	return count;
// }
void Solution::bfs(vector<vector<char>>& grid,
                   deque<pair<int, int>>& coordinates, int m, int n) {
    int x, y;
...
int Solution::numIslands(vector<vector<char>>& grid) // UnionFind *
{
    ...
    // 另一种计算方法，ans 即返回值，注意一个岛屿只有一个点使等式成立
    return s.size();
}
...
//	return result;
// }
vector<int> Solution::nextGreaterElement(vector<int>& nums1,
                                         vector<int>& nums2) { // 单调栈解法
    ...
    for (int i = n - 1; i >= 0; --i) {
        while (!numsStack.empty() &&
               numsStack.top() < nums2[i]) // 维护一个单调栈
            numsStack.pop();
        if (!numsStack.empty())
            numsMap[nums2[i]] =
                numsStack.top(); // 每个元素的右边最大值存入 hash 表
        else
            numsMap[nums2[i]] = -1;
...
int Solution::chalkReplacer(vector<int>& chalk,
                            int k) { // leetcode 1894 9/12/22
    int n = chalk.size(); // 思路还行，时间、空间效率一般，C++ 11 可以使用 long
...
int Solution::minSpeedOnTime(vector<int>& dist,
                             double hour) { // leetcode 1870 9/10/22
    int n = dist.size();
    if (n - 1 >= hour)
        return -1;

    int max = dist[0], min = 1,
        speed; // 因为 hour 精确到小数点后两位，所以距离最大值为最大值的 100 倍
...
int Solution::networkDelayTime(vector<vector<int>>& times, int n,
    ...
    int result =
        *max_element(++dis.begin(), dis.end()); // 别忘了跳过第 0 个元素
    return result == inf ? -1 : result;
}

bool Solution::containsCycle(
    vector<vector<char>>& grid) { // leetcode 1559 9/8/22 ***
    int m = grid.size(), n = grid[0].size();
    if (m == 1 || n == 1)
        return false;
...
vector<vector<int>> Solution::floodFill(vector<vector<int>>& image, int sr,
    ...
    vector<vector<int>> sign(
        ...
        vector<int>(
            n,
            0)); // 不需要标志数组，直接修改image颜色，见参考代码（因为传的对象引用，修改会改变函数外对象的值）
...
int Solution::minPathSum(
    vector<vector<int>>& grid) { // leetcode 64	// 进阶版，允许各个方向的移动 *
    int m = grid.size(), n = grid[0].size();
    vector<vector<int>> dis(
        m, vector<int>(n, 0)); // m 行 n 列的值全为 0 的二维向量
...
//		for (int j = 1; j < i; ++j) t[j] = ans[i - 1][j] + ans[i - 1][j
//- 1]; 		ans.push_back(t);	// push_back更快
//	}
//	return ans;
// }

vector<int> Solution::searchRange(vector<int>& nums,
                                  int target) { // leetcode 34 *
    int len = nums.size();
