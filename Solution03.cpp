...
vector<vector<int> > Solution03::updateMatrix(vector<vector<int> >& mat)
{
...
void Solution03::bfs_542_1(int x, int y, vector<vector<int> >& mat, vector<vector<int> >& vis, vector<vector<int> >& result)
{
    int m = mat.size(), n = mat[0].size();
...
void Solution03::bfs_542_2(vector<vector<int> >& mat, vector<vector<int> >& vis, vector<vector<int> >& result)
{
    int m = mat.size(), n = mat[0].size();
    vector<int> dis = { 0, 1, 0, -1, 0 };
...
vector<vector<int>> Solution03::updateMatrix_(vector<vector<int>>& mat)
{
    int m = mat.size(), n = mat[0].size();
...
vector<vector<int>> Solution03::highestPeak(vector<vector<int>>& isWater)
{
...
int Solution03::minKnightMoves(int x, int y)
{
    x += 310;
    y += 310;
...
int Solution03::minKnightMoves_(int x, int y)
{
    if (x == 0 && y == 0) return 0;
    x += 310;
    y += 310;
...
int Solution03::extend_1197(unordered_map<int, int>& m1, unordered_map<int, int>& m2, deque<pair<int, int>>& q)
{
    vector<pair<int, int> > dis = { { 1, 2 }, { 1, -2 }, { 2, 1 }, { 2, -1 }, { -1, 2 }, { -1, -2 }, { -2, 1 }, { -2, -1 } };
    for (int i = q.size(); i; --i)
    {
...
int Solution03::shortestPathBinaryMatrix(vector<vector<int>>& grid)
{
    int n = grid.size();
...
int Solution03::extend_1091(unordered_map<int, int>& m_oppo, unordered_map<int, int>& m, deque<pair<int, int> >& q, int n, vector<vector<int> >& grid)
{
...
int Solution03::nearestExit(vector<vector<char>>& maze, vector<int>& entrance)
{
...
int Solution03::shortestPath(vector<vector<int>>& grid, int k)
{
...
int Solution03::openLock(vector<string>& deadends, string target)
{
    int n = deadends.size();
...
int Solution03::openLock_(vector<string>& deadends, string target)
{
    unordered_set<string> dead(deadends.begin(), deadends.end());
    if (dead.count("0000"))
        return -1;
...
int Solution03::f(string state, string& target)
{
    int result = 0;
    for (int i = 0; i < 4; ++i)
    {
...
int Solution03::ladderLength(string beginWord, string endWord, vector<string>& wordList)
{
...
int Solution03::minimumOperations(vector<int>& nums, int start, int goal)
{
    if (start == goal)
        return 0;
...
int Solution03::extend_2059(unordered_map<int, int>& m_q, unordered_map<int, int>& m, deque<int>& q, vector<int>& nums)
{
    for (int i = q.size(); i; --i)
    {
        auto p = q.front();
...
int Solution03::slidingPuzzle(vector<vector<int>>& board)
{
    int m = 2, n = 3;
    string start, seq;
    string end = "123450";
...
bool Solution03::check(string& str)
{
    int len = str.size();
    int cnt = 0;
    for (int i = 0; i < len; ++i)
...
int Solution03::f_773(string& state)
{
...
int Solution03::shortestPathLength(vector<vector<int>>& graph)
{
    int n = graph.size();
...
int Solution03::cutOffTree(vector<vector<int>>& forest)
{
...
void Solution03::maxHeapify(vector<vector<int> >& v, int heapSize, int i) // 维护堆（大根堆）的性质
{
    while (true)
    {
...
inline int Solution03::f_675(int x1, int y1, int x2, int y2)
{
    return abs(x1 - x2) + abs(y1 - y2);
}

int Solution03::minCost(vector<vector<int>>& grid)
{
    int m = grid.size(), n = grid[0].size();
...
int Solution03::minimumObstacles(vector<vector<int>>& grid)
{
    int m = grid.size(), n = grid[0].size();
...
bool Solution03::hasPath(vector<vector<int>>& maze, vector<int>& start, vector<int>& destination)
{
...
bool Solution03::exist(vector<vector<char>>& board, string word)
{
...
bool Solution03::dfs_79(int i, int j, int idx, int m, int n, int len, vector<vector<char> >& board, string& word)
{
...
int Solution03::getMaximumGold(vector<vector<int>>& grid)
{
...
int Solution03::dfs_1219(vector<vector<int> >& grid, int m, int n, int x, int y)
{
...
bool Solution03::makesquare(vector<int>& matchsticks)
{
...
bool Solution03::dfs_473(vector<int>& matchsticks, int len, int idx, int count, int sum, int avg)
{
...
bool Solution03::makesquare_(vector<int>& matchsticks)
{
...
bool Solution03::dfs_473_(int idx, int avg, int len, vector<int>& matchsticks, vector<int>& edges)
{
    if (idx == len) return true;
    for (int i = 0; i < 4; ++i)
    {
...
bool Solution03::makesquare__(vector<int>& matchsticks)
{
...
bool Solution03::canPartitionKSubsets(vector<int>& nums, int k)
{
...
int Solution03::minimumTimeRequired(vector<int>& jobs, int k)
{
...
int Solution03::dfs_1723(vector<int>& jobs, vector<int>& edges, int len, int k, int idx, int ret)
{
    ++count_dfs_1723;
    if (idx == len)
    {
...
int Solution03::distributeCookies(vector<int>& cookies, int k)
{
...
void Solution03::dfs_2305(vector<int>& cookies, vector<int>& edges, int idx, int len, int k)
{
    if (idx == len)
...
int Solution03::longestIncreasingPath(vector<vector<int>>& matrix)
{
    int ret = 1;
...
int Solution03::dfs_329(int x, int y)
{
...
int Solution03::countPaths(vector<vector<int>>& grid)
{
...
ull Solution03::dfs_2328(int x, int y)
{
    if (memo[x][y]) return memo[x][y];
    int sum = 1;
    for (int i = 0; i < 4; ++i)
...
bool Solution03::canWin(string currentState)
{
    len = currentState.size();
    return dfs_294(currentState);
}

bool Solution03::dfs_294(string& currentState)
{
...
bool Solution03::canWin_(string currentState)
{
    len = currentState.size();
    ll mask = 0;
    for (int i = 0; i < len; ++i)
...
void Solution03::dfs_294_(ll mask)
{
    for (int i = 0; i < len - 1; ++i)
...
bool Solution03::canWin__(string currentState)
{
    len = currentState.size();
    sg.resize(len + 1, -1);
...
int Solution03::countRoutes(vector<int>& locations, int start, int finish, int fuel)
{
    len = locations.size();
...
void Solution03::dfs_1575(int idx, int finish, int fuel, vector<int>& locations, vector<vector<int> >& fuels)
{
...
int Solution03::ways(vector<string>& pizza, int k)
{
...
int Solution03::dfs_1444(int x, int y, int k)
{
